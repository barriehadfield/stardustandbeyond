...
function peopleReport(report) {
  ...
  md += `- Ambiguous bare names resolved by co-occurrence: **${report.resolvedByCooccurrence.length}**\n`;
  md += `- Still unresolved: **${report.unresolved.length}**\n`;
  md += `- People without a surname (in photos): **${report.surnameless.length}**\n`;
  md += `- Shared first names: **${report.sharedNames.length}**\n`;
  md += `- Unmatched candidate tokens: **${report.unmatched.length}**\n\n`;

  md += "## People without a surname\n\nRoster entries that appear in photos but have no surname yet, with who they most co-occur with (a clue to their identity). Add a surname by renaming them in `source/people.md` or the corrections console.\n\n";
  md += "| Person | Photos | Most often photographed with |\n|---|---|---|\n";
  for (const s of report.surnameless)
    md += `| ${s.display} | ${s.count} | ${s.topCooc.join(", ") || "-"} |\n`;

  md += "\n## Shared first names\n\nPeople who share a first name. Bare mentions are resolved by co-occurrence (who they're pictured with, in that decade), then by `Primary`, then left unresolved.\n\n";
  md += "| First name | by co-occurrence | by Primary | unresolved | Primary | People sharing it |\n|---|---|---|---|---|---|\n";
  for (const s of report.sharedNames)
    md += `| ${s.first} | ${s.byCooccurrence} | ${s.byPrimary} | ${s.unresolved} | ${s.primary || "_(none)_"} | ${s.candidates.join("; ")} |\n`;

  md += `\n## Ambiguous names resolved by co-occurrence (${report.resolvedByCooccurrence.length})\n\nA bare first name attributed by who else is in the photo, within the same decade. Spot-check; add an override if any is wrong.\n\n`;
  for (const a of report.resolvedByCooccurrence.slice(0, 200))
    md += `- **${a.first}** -> **${a.chosen}** in \`${a.section}/${a.file}\` (of: ${a.candidates.join(", ")})\n  - caption: "${a.caption}"\n  - ${url(a.section, a.file)}\n`;

  md += `\n## Still unresolved (${report.unresolved.length})\n\nA bare shared first name with no co-occurrence signal and no Primary. Add an override, or set a Primary.\n\n`;
